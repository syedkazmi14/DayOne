...
const statusBar = document.querySelector<HTMLElement>('[role="progressbar"]')
ok(!!statusBar && (statusBar.getAttribute('aria-valuetext') ?? '').includes('THE INBOX'), 'episode status bar reports the current act')
ok(!!statusBar?.querySelector('img[src="/statusBars/portalGun.png"]'), 'Rick and Morty episodes use the portal-gun status bar')
{
  const bars = await import('../src/components/statusBars')
  const { PortalGunBar } = await import('../src/components/statusBars/PortalGunBar')
  const { ChickenFightBar } = await import('../src/components/statusBars/ChickenFightBar')
  const { ActRailBar } = await import('../src/components/statusBars/ActRailBar')
  ok(bars.statusBarFor('rick-and-morty') === PortalGunBar, 'Rick and Morty map to the portal-gun bar')
  ok(bars.statusBarFor('family-guy') === ChickenFightBar, 'Family Guy maps to the chicken-fight bar')
  ok(bars.statusBarFor('the-simpsons') === ActRailBar && bars.statusBarFor(null) === ActRailBar, 'shows without a design fall back to the act rail')
}
...
await flush(400)
ok(document.querySelector('h1')?.textContent === 'Results', 'results screen')
ok(!has('episode complete'), 'results drop the redundant episode-complete label')
...
ok(has('coach summary'), 'coach section')
await act(async () => { await sleep(900) })
await flush(300)
ok(has('next episode focuses on'), 'next-episode plan rendered')
const coachHeadline = () => document.querySelector('section h2')?.textContent?.trim() ?? ''
const goodRunHeadline = coachHeadline()
ok(goodRunHeadline !== '', 'coach headline rendered')
...
await click('see your results')
await flush(1400)
ok(document.querySelector('h1')?.textContent === 'Results', 'results after a bad run')
ok(has('Suppressed a security incident') || has('Clicked an unverified link'), 'ledger records the failures')
ok(coachHeadline() !== '' && coachHeadline() !== goodRunHeadline, `coach reads the bad run differently ("${coachHeadline()}" vs "${goodRunHeadline}")`)
...
ok(has('recommended:'), 'results recommend an admin-built episode aimed at the weakness — employees never generate')
